...
import { CheckCircle2, AlertCircle, Timer, XCircle, Edit2, Trash2, Link, Upload, Lock } from 'lucide-react';

interface Props {
  tasks: TaskRecord[];
  onEdit?: (task: TaskRecord) => void;
  onDelete?: (taskId: string) => void;
  // New prop to determine if a specific task row is read-only
  isReadOnly?: (task: TaskRecord) => boolean;
}

const TaskTable: React.FC<Props> = ({ tasks, onEdit, onDelete, isReadOnly }) => {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[1200px]">
          ...
          <tbody className="divide-y divide-gray-50">
            {tasks.map((task, idx) => {
              const readOnly = isReadOnly?.(task) || false;
                ...
                <tr key={task.id || idx} className="hover:bg-[#8A7AB5]/5 transition-colors group">
                  ...
                  {showActions && (
                    <td className="px-5 py-3 text-right">
                      {readOnly ? (
                        <div className="flex items-center justify-end gap-2 text-slate-300 pr-1.5" title="Historical Record (Locked)">
                          <Lock size={12} />
                        </div>
                      ) : (
