...
const ActionOverlay = ({ type, preSelectedEmployee, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. If Manager & creating task/project -> Call Backend API
      if (manager && manager.managerId && (type === 'NEW_PROJECT' || type === 'ASSIGN_TASK')) {
        const payload = {
          managerId: manager.managerId,
          title: formData.taskName,
          description: formData.taskDescription,
          priority: formData.priority,
          deadline: formData.dueDate, // API handles empty string or null
          metadata: {
            projectName: formData.projectId || 'General' // In manual mode, projectId holds the text input
          }
        };

        // Step 1: Create Task
        const createResponse = await taskService.createTask(payload);
        const createdTask = createResponse.data?.task;

        // Step 2: Assign Task (if employee selected)
        if (createdTask && createdTask.taskId && formData.employeeId) {
          await taskService.assignTask(createdTask.taskId, {
            managerId: manager.managerId,
      ...
      // 2. Fallback / Mock logic for other flows (e.g. Employee doing something or demo mode)
      else {
        ...
        // Fallback project name if no project list
        const projectName = selectedProject?.name || formData.projectId || 'General';
        ...
        if (selectedEmployee) {
          const assignment = {
            ...
            projectId: formData.projectId || 'PROJ-DEFAULT',
            assignedTo: selectedEmployee.name || 'Unknown',
            assignedToId: selectedEmployee.id,
            assignedBy: 'Alex Sterling', // Mock user
      ...
      // Fail silently in UI or alert user? For now keeping UI consistent (no error state design provided)
                  ...
                  {preSelectedEmployee ? (
                    <div className="p-4 bg-bgAudvik border border-borderAudvik rounded-lg flex items-center gap-3">
                       <div className="w-10 h-10 bg-white border border-borderAudvik rounded-lg flex items-center justify-center font-bold text-primary">
                         {preSelectedEmployee.name ? preSelectedEmployee.name[0] : '?'}
                       </div>
                       <div>
                         <p className="text-sm font-bold text-slateBrand">{preSelectedEmployee.name || 'Unknown'}</p>
                ...
                <div className="space-y-1 md:col-span-2">
                  ...
                  {projects.length > 0 ? (
                    ...
                    <input 
                      type="text"
                      name="projectId"
                      placeholder="Enter Project Name (Manual Mode)"
                      className="w-full bg-white border border-borderAudvik rounded-lg p-3 text-sm font-semibold outline-none focus:border-primary transition-all"
                      onChange={handleInputChange}
                      required
                    />
