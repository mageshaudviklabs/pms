...
import { assignTasksFromExcel, saveAssignmentLogs } from '../../services/assignmentService';
...
const ExcelImportModal = ({ onClose, onAssignmentComplete }) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const tasks = jsonData.map((row, index) => ({
          ...
          taskName: row['Task Name'] || row['Task'] || 'Untitled Task',
          taskDescription: row['Task Description'] || row['Description'] || '',
          projectName: row['Project Name'] || row['Project'] || 'General',
  ...
  const confirmImport = () => {
    const newAssignments = assignTasksFromExcel(parsedTasks);
    setAssignments(newAssignments);
    saveAssignmentLogs(newAssignments);
    setStep('success');
    
    setTimeout(() => {
      onAssignmentComplete && onAssignmentComplete(newAssignments);
      onClose();
    }, 2000);
  };
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end pt-4 border-t border-borderDiv gap-3">
                ...
                <button 
                  ...
                  disabled={!file}
                  className={`px-8 py-3 rounded-xl font-bold transition-all ${file ? 'bg-primary text-white hover:bg-primaryDark' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
                >
                  Parse & Preview
                </button>
              </div>
          ...
          {step === 'preview' && (
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-borderDiv">
                ...
                <button onClick={confirmImport} className="px-8 py-3 bg-primary text-white rounded-xl font-bold hover:bg-primaryDark transition-all">
                  Auto-Assign Tasks
                </button>
              </div>
            </div>
